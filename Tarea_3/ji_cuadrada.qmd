...
for (n in gl) {
  ...
  plot(muestras, medias_muestrales, type = "l", col = "plum3",
...
for (n in gl) {
  ...
  plot(muestras, varianzas_muestrales, type = "l", col = "orange2",
...
for (n in tamaños_muestra) {
  ...
  hist(muestras, breaks = 30, probability = TRUE, col = "dodgerblue",
...
n_sim <- 10000
gl <- 10 
muestras <- replicate(n_sim, sum(rnorm(gl)^2))
df_muestras <- data.frame(x = muestras)

suma_ji <- ggplot(df_muestras, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightgreen", color = "black", alpha = 0.7) +
  geom_function(fun = function(x) dchisq(x, df = gl), aes(color = "Ji-cuadrada Teórica"), size = 1.4) +
  scale_color_manual(name = "Distribución",
                     values = c("Ji-cuadrada Teórica" = "red"),
                     breaks = c("Ji-cuadrada Teórica")) +
  labs(
    title = paste("Verificación de la propiedad de Aditividad de la distribución ji-cuadrada"),
    subtitle = "Suma de variables aleatorias normales estándar elevadas al cuadrado",
    x = "Valor", y = "Densidad"
  ) +
  theme_minimal() + 
  theme(
...
par(mfrow = c(2, 2))
for (n in gl) {
  muestras <- rchisq(n_sim, df = n)
  hist(muestras, breaks = 30, probability = TRUE, col = "turquoise",
       main = paste("Aproximación Normal de la Distribución ji- cuadrada para n =", n),
       xlab = "x", ylab = "Densidad")
  dens <- density(muestras)
...
suma_ji <- ggplot(df_suma, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "seashell2", color = "black", alpha = 0.7) +
