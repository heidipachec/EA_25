...
t_fuera_spec <- sum(temperaturas < lim_inf | temperaturas > lim_sup)
pc_fuera <- (t_fuera_spec / n_mediciones) * 100
cat("El porcentaje de tiempo donde la temperatura está fuera de especificación es:", round(pc_fuera, 2), "%\n")
```
...
mu2 <- 355
temperaturas_2 <- rnorm(n_mediciones, mean = mu2, sd = sigma)
t_fuera_spec_2 <- sum(temperaturas_2 < lim_inf | temperaturas_2 > lim_sup)
pc_fuera_2 <- (t_fuera_spec_2 / n_mediciones) * 100
cat("El porcentaje de tiempo donde la temperatura está fuera de especificación con μ=355°C es:", round(pc_fuera_2, 2), "%\n")
```
...
z <- qnorm(0.999)
sigma_necesario <- (lim_sup - mu) / z
cat("El σ necesario para tener 99.9% de productos dentro de especificación es:", round(sigma_necesario, 2), "°C\n")
```
:::
...
pc_mas_60000 <- 1- pnorm(60000, mean = mu, sd = sigma) 
cat("La probabilidad de tener ventas superiores a 60,000 pesos es:", round(pc_mas_60000*100, 2), "%\n")
```
...
ventas_menos_30000 <- pnorm(30000, mean = mu, sd = sigma) * n_meses
cat("3. Se espera tener ventas por debajo de 30,000 pesos en:", round(ventas_menos_30000, 2), "meses\n")
```
...
q1 <- quantile(ventas, 0.25)
q3 <- quantile(ventas, 0.75)
iqr <- q3 - q1
cat("El rango intercuartílico de las ventas es:", round(iqr, 2), "pesos\n")
```
...
pc_mas_45000 <- mean(ventas > 45000) * 100
cat("El porcentaje de meses que se cumple la meta de superar 45,000 pesos es:", round(pc_mas_45000, 2), "%\n")
```
:::
...
garantia_h_95 <- qnorm(0.05, mean = mu, sd = sigma)
cat("La garantía que cubre al 95% de las baterías es de:", round(garantia_h_95, 2), "horas\n")
```
...
n_vendidas <- 10000
pc_falla_900 <- pnorm(900, mean = mu, sd = sigma)
n_fallas_esperadas <- pc_falla_900 * n_vendidas
cat("Se espera que fallen:", round(n_fallas_esperadas, 2), "baterías con garantía de 900 horas\n")
```
...
cat("Media teórica:", mu, "horas\n")
cat("Mediana teórica:", mu, "horas\n")
```
...
cat("El percentil 10 de la distribución es:", round(percentil_10, 2), "horas\n")
```
:::
...
pc_menos5_mas5 <- mean(errores >= -5 & errores <= 5) * 100
cat("El porcentaje de errores que está entre -5 y +5 unidades es:", round(pc_menos5_mas5, 2), "%\n")
```
...
pc_mas_6 <- 2 * pnorm(-6, mean = mu, sd = sigma)
cat("La probabilidad de que un error sea mayor a 6 unidades en valor absoluto es:", round(pc_mas_6*100, 2), "%\n")
```
...
pc_preciso <- mean(abs(errores) <= 3) * 100
cat("El porcentaje de mediciones que son precisas (error ≤ 3 unidades) es:", round(pc_preciso, 2), "%\n")
```
...
cat("El coeficiente de asimetría de la distribución es:", round(asimetria, 4), "\n")
```
:::
...
pc_A_mas_B <- mean(grupo_A > mu_B) * 100
cat("El porcentaje de estudiantes del Grupo A que supera la media del Grupo B es:", round(pc_A_mas_B, 2), "%\n")
```
...
pc_B_menos_70 <- pnorm(70, mean = mu_B, sd = sigma_B) * 100
cat("La probabilidad de que un estudiante del Grupo B tenga puntuación menor a 70 es:", round(pc_B_menos_70, 2), "%\n")
```
...
ggplot(df_examen, aes(x = puntuacion, fill = grupo)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mu_A, sd = sigma_A), color = "blue", size = 1) +
  stat_function(fun = dnorm, args = list(mean = mu_B, sd = sigma_B), color = "red", size = 1) +
...
cat("El 10% superior del Grupo A tiene puntuación mayor a:", round(p90_A, 2), "\n")
cat("El 10% superior del Grupo B tiene puntuación mayor a:", round(p90_B, 2), "\n")
