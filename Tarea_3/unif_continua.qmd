...
tiempos_carga <- runif(n_carga_sim, min = a, max = b)

ggplot(data.frame(tiempo = tiempos_carga), aes(x = tiempo)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, 
                 fill = "cyan4", color = "black", alpha = 0.7) +
  stat_function(fun = dunif, args = list(min = a, max = b), 
                color = "red", size = 1, linetype = "dashed") +
  labs(title = "Distribución de Tiempos de Carga Simulados con Densidad Teórica",
...
cat("El percentil 75 de los tiempos de carga es:", round(percentil_75, 2), "segundos\n")
```
...
cat("La probabilidad de que una página cargue en menos de 4 segundos es:", round(prob_menos_4, 4), "\n")
```
:::
...
a <- 70
b <- 100
n_estudiantes_sim <- 500

calificaciones <- runif(n_estudiantes_sim, min = a, max = b)

pc_mas_85 <- mean(calificaciones > 85) * 100
cat("El porcentaje de estudiantes con calificación superior a 85 es:", round(pc_mas_85, 2), "%\n")
```
...
pc_categorias <- prop.table(tabla_categorias) * 100
...
#| fig-width: 12
 ...
 df_calif <- data.frame(calificaciones = calificaciones, categoria = categorias)
 ggplot(df_calif, aes(x = calificaciones, fill = categoria)) +
  ...
  labs(title = "Distribución de Calificaciones por Categorías",
       x = "Calificación",
       y = "Estudiantes",
       fill = "Categoría") +
  theme_minimal()
...
n_medias <- 800
resultados_tcl_trunc <- list()
for (n in tamaños_muestra) {
  medias <- replicate(n_medias, mean(sample(muestra_truncada(n), n)))
  resultados_tcl_trunc[[as.character(n)]] <- data.frame(
    medias = medias,
    n = n
  )
}
df_tcl_trunc <- do.call(rbind, resultados_tcl_trunc)
df_tcl_trunc$n_factor <- factor(paste("n =", df_tcl_trunc$n))

ggplot(df_tcl_trunc, aes(x = medias)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, 
                 fill = "paleturquoise1", color = "black", alpha = 0.7) +
  geom_vline(xintercept = mean(muestra_truncada(10000)), color = "red", size = 1, linetype = "dashed") +
  facet_wrap(~n_factor, scales = "free_y", ncol = 3) +
  labs(title = "TCL con Distribución Uniforme Truncada (>0.3)",
       x = "Media muestral",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
data_tcl_trunc <- df_tcl_trunc %>%
  group_by(n) %>%
  summarise(
    Asimetría = skewness(medias),
    Curtosis = kurtosis(medias),
    .groups = 'drop'
  )
kable(data_tcl_trunc, 
      caption = "Asimetría y Curtosis de las Medias Muestrales",
      digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
circulo <- (x^2 + y^2) <= 1
n_dentro <- sum(circulo)
pi_estimado <- (n_dentro / n) * 4
cat("La estimación de π usando simulación Monte Carlo con", n, "puntos es:", round(pi_estimado, 6), "\n")
```
...
df_pi <- data.frame(x = x, y = y, dentro = circulo)
ggplot(df_pi, aes(x = x, y = y, color = dentro)) +
  geom_point(alpha = 0.5, size = 1) +
  coord_fixed() +
  labs(title = "Simulación Monte Carlo para Estimación de π",
       x = "X",
       y = "Y",
       color = "Círculo") +
  theme_minimal() +
