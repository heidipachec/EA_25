...
p_cara <- 0.5
k_fracaso <- 4
prob_5 <- dgeom(k_fracaso, p_cara)
cat("La probabilidad de necesitar exactamente 5 lanzamientos es:", round(prob_5, 4), "\n")
```
...
p_cara <- 0.5
prob_mas_3 <- 1 - pgeom(2, p_cara)
cat("La probabilidad de necesitar más de 3 lanzamientos es:", round(prob_mas_3, 4), "\n")
```
...
var_lanzamientos <- (1 - p_cara) / (p_cara^2)
cat("La media del número de lanzamientos es:", media_lanzamientos, "\n")
cat("La varianza del número de lanzamientos es:", var_lanzamientos, "\n")
```
:::
...
p_anotar <- 0.7
k_intentos <- 2
prob_3 <- dgeom(k_intentos, p_anotar)
cat("La probabilidad de anotar en el tercer intento por primera vez es:", round(prob_3, 4), "\n")
```
...
cat("El número esperado de intentos para anotar es:", round(media_intentos, 4), "\n")
```
...
prob_2_5 <- sum(dgeom(1:4, p_anotar))
cat("La probabilidad de necesitar entre 2 y 5 intentos es:", round(prob_2_5, 4), "\n")
```
:::
...
p_defecto <- 0.02
media_inspecciones <- (1 - p_defecto) / p_defecto
cat("El número esperado de inspecciones hasta encontrar la primera pieza defectuosa es:", round(media_inspecciones, 4), "\n")
```
...
cat("La probabilidad de que la primera pieza defectuosa aparezca después de la pieza número 100 es:", round(prob_despues_100, 4), "\n")
```
