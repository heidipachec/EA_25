...
# Máquina que produce piezas defectuosas 
p_maquina <- 0.02
...
bernoulli_maquina <- data.frame(
  ...
  Valor = c(0, 1),
  Probabilidad = c(1-p_maquina, p_maquina)
)
...
```{r}
#| code-fold: true
#| label: grafico-calidad maquina
#| fig-cap: "Distribución Binomial - Control de calidad"

ggplot(calidad_data[1:8, ], aes(x = Defectos, y = Probabilidad)) +
  geom_col(fill = "dodgerblue", alpha = 0.7, width = 0.8) +
  geom_text(aes(label = round(Probabilidad, 4)), vjust = -0.5, size = 3) +
  geom_vline(xintercept = media_defectos, color = "red", linetype = "dashed", size = 1) +
  annotate("text", x = media_defectos + 1.2, y = max(prob_defectos) * 0.6, 
           label = paste("Media =", media_defectos), color = "red") +
...
bernoulli_examen <- data.frame(
  ...
  Valor = c(0, 1),
  Probabilidad = c(1-p_examen, p_examen)
)

kable(bernoulli_examen, 
      caption = "Distribución Bernoulli - Examen de opción multiple con 4 opciones (p = 0.25)",
      align = "ccc") %>%
  kable_styling(bootstrap_options = "striped")
```
...
```{r}
#| code-fold: true
cat("La probabilidad de acertar es de:", p_examen, "\n")
```
...
varianza_examen <- p_examen * (1 - p_examen)
...
cat("La probabilidad de obtener exactamente 8 caras es:", round(prob_8_caras, 4), "\n")
```
...
prob_min_12 <- 1 - pbinom(11, n_lanzamientos, p_cara)
cat("La probabilidad de obtener al menos 12 caras:", round(prob_min_12, 4), "\n")
```
...
prob_curen_18 <- dbinom(18, n, p)
cat("La probabilidad de que se curen exactamente 18 pacientes es:", round(prob_curen_18, 4), "\n")
```
...
prob_curen_15_19 <- pbinom(19, n, p) - pbinom(15, n, p)
cat("La probabilidad de que se curen entre 15 y 19 pacientes es:", round(prob_curen_15_19, 4), "\n")
```
...
pacientes_curados <- n*p
cat("Se espera que se curen:", pacientes_curados, "\n")
```
:::
...
```{r}
#| label: simulacion-binomial-suma
...
Z_sim <- X + Y
Z_teo <- rbinom(n_sim, n + m, p)

cat("Media suma simulada:", mean(Z_sim), "\n")
cat("Media binomial teórica:", mean(Z_teo), "\n")
cat("Varianza suma simulada:", var(Z_sim), "\n")
cat("Varianza binomial teórica:", var(Z_teo), "\n")

tabla_sim <- table(Z_sim) / n_sim
tabla_teo <- table(Z_teo) / n_sim
k <- sort(unique(c(names(tabla_sim), names(tabla_teo))))
freq_sim <- as.numeric(tabla_sim[as.character(k)])
freq_teo <- as.numeric(tabla_teo[as.character(k)])

comparacion <- data.frame(
  k = as.integer(k),
  Freq_SumaSimulada = replace(freq_sim, is.na(freq_sim), 0),
  Freq_BinTeorica = replace(freq_teo, is.na(freq_teo), 0)
)

comp_long <- comparacion %>%
  pivot_longer(cols = c(Freq_SumaSimulada, Freq_BinTeorica),
               names_to = "Tipo", values_to = "Frecuencia") %>%
  mutate(Tipo = recode(Tipo,
                       "Freq_SumaSimulada" = "Suma Simulada",
                       "Freq_BinTeorica" = "Binomial Teórica"))

ggplot(comp_long, aes(x = k, y = Frecuencia, fill = Tipo)) +
  geom_col(position = "dodge", alpha = 0.7) +
  scale_fill_manual(values = c("Binomial Teórica" = "blue", "Suma Simulada" = "red")) +
  labs(title = paste("Comparación Binomial (10, 0.5) - 10000 simulaciones"),
       x = "Número de Éxitos",
       y = "Frecuencia",
       fill = "Distribución") +
  theme_minimal() +
