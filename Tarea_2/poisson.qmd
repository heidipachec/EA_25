...
lambda_llamadas <- 6
prob_4 <- dpois(4, lambda_llamadas)
cat("1. P(X = 4 llamadas) =", round(prob_4, 4), "(", round(prob_4*100, 2), "%)\n")
```
...
lambda_30seg <- lambda_llamadas * 0.5
prob_0_30seg <- dpois(0, lambda_30seg)
cat("2. P(X = 0 llamadas en 30 seg) =", round(prob_0_30seg, 4), "(", round(prob_0_30seg*100, 2), "%)\n")
```
...
lambda_hora <- lambda_llamadas * 60
cat("3. Llamadas esperadas en una hora =", lambda_hora, "\n")
```
...
lambda_2min <- lambda_llamadas * 2
prob_8_12 <- ppois(12, lambda_2min) - ppois(7, lambda_2min)
cat("4. P(8 ≤ X ≤ 12 en 2 min) =", round(prob_8_12, 4), "(", round(prob_8_12*100, 2), "%)\n")
```
:::
...
lambda_defectos_500 <- promedio_defectos * 500
prob_8_defectos <- dpois(defectos, lambda_defectos_500)
cat("1. P(X = 8 defectos en 500 productos) =", round(prob_8_defectos, 4), "(", round(prob_8_defectos*100, 2), "%)\n")
```
...
prob_10_mas <- 1 - ppois(9, lambda_defectos_500)
cat("2. P(X ≥ 10 defectos en 500 productos) =", round(prob_10_mas, 4), "(", round(prob_10_mas*100, 2), "%)\n")
```
...
prob_un_defecto <- 0.95
prob_0_defectos <- 1 - prob_un_defecto
lambda_producto <- -log(prob_0_defectos)
productos_necesarios <- lambda_producto / promedio_defectos
cat("3. Productos necesarios para que la probabilidad de tener al menos un defecto sea = 0.95:", round(productos_necesarios), "\n")
```
...
promedio_defectos_ajuste <- 1.2 / 100
lambda_defectos_500_ajuste <- promedio_defectos_ajuste * 500
prob_mas_10_ajuste <- 1 - ppois(9, lambda_defectos_500_ajuste)
cat("4. Si se mejora el proceso P(X ≥ 10 defectos en 500 productos) =", round(prob_10_mas, 4), "(", round(prob_10_mas*100, 2), "%)\n")
```
:::
...
lambda_pacientes <- 2.5
prob_3 <- dpois(3, lambda_pacientes)
cat("1. P(X = 3 pacientes en 1 hora) =", round(prob_3, 4), "(", round(prob_3*100, 2), "%)\n")
```
...
lambda_2horas <- lambda_pacientes * 2
prob_0_2horas <- dpois(0, lambda_2horas)
cat("2. P(X = 0 pacientes en 2 horas) =", round(prob_0_2horas, 4), "(", round(prob_0_2horas*100, 2), "%)\n")
```
...
lambda_4horas <- lambda_pacientes * 4
prob_saturacion <- 1 - ppois(8, lambda_4horas)
cat("3. La probabilidad de superar la capacidad de atender 8 pacientes por tuno de 4 horas del hospital es:", round(prob_saturacion, 4), "(", round(prob_saturacion*100, 2), "%)\n")
```
...
capacidad_minima <- 0
while(TRUE) {
  prob_saturacion_min <- 1 - ppois(capacidad_minima, lambda_4horas)
  if(prob_saturacion_min < 0.10) {
    break
  }
  capacidad_minima <- capacidad_minima + 1
}
cat("4. La capacidad mínima para que la probabilidad de saturación sea menor al 10% en un turno de 4 horas es:", capacidad_minima, "\n")
```
:::
...
tamaños_muestra_tcl <- c(5, 15, 30, 60, 120)
num_muestras_tcl <- 3000
cat("Distribución Poisson: (λ =", lambda_tcl_poisson,")\n")
cat("Tamaños de muestra:", paste(tamaños_muestra_tcl, collapse = ", "), "\n")
cat("Número de muestras por tamaño:", num_muestras_tcl, "\n\n")
cat("Parámetros teóricos de la distribución poisson:\n")
cat("E[X] =", lambda_tcl_poisson, "\n")
cat("Var(X) =", lambda_tcl_poisson, "\n")

# Función para generar medias muestrales
generar_medias_muestrales_poisson <- function(n, num_muestras, lambda) {
  medias <- numeric(num_muestras)
  for(i in 1:num_muestras) {
    muestra <- rpois(n, lambda)
    medias[i] <- mean(muestra)
...
for(i in seq_along(tamaños_muestra_tcl)) {
  n <- tamaños_muestra_tcl[i]
  medias_muestrales_poisson[[i]] <- generar_medias_muestrales_poisson(n, num_muestras_tcl, lambda_tcl_poisson)
...
data_tcl_poisson <- do.call(rbind, lapply(seq_along(medias_muestrales_poisson), function(i) {
  data.frame(
    ...
    n = tamaños_muestra_tcl[i],
    n_label = paste("n =", tamaños_muestra_tcl[i])
  )
}))
data_tcl_poisson$n_label <- factor(data_tcl_poisson$n_label, levels = c("n = 5", "n = 15", "n = 30", "n = 60", "n = 120"))
cat("Muestras generadas exitosamente.\n")
cat("Total de observaciones:", nrow(data_tcl_poisson), "\n")

# Calcular estadísticas para cada tamaño de muestra
estadisticas_tcl_poisson <- data.frame(
  n = tamaños_muestra_tcl,
  Media_Observada = sapply(medias_muestrales_poisson, mean),
  Media_Teorica = rep(lambda_tcl_poisson, length(tamaños_muestra_tcl)),
  Var_Observada = sapply(medias_muestrales_poisson, var),
  Var_Teorica = lambda_tcl_poisson / tamaños_muestra_tcl,
  SD_Observada = sapply(medias_muestrales_poisson, sd),
  SD_Teorica = sqrt(lambda_tcl_poisson / tamaños_muestra_tcl)
)

# Calcular errores
estadisticas_tcl_poisson <- estadisticas_tcl_poisson |> mutate(
  Error_Media = abs(Media_Observada - Media_Teorica),
  Error_Var = abs(Var_Observada - Var_Teorica),
  Error_SD = abs(SD_Observada - SD_Teorica))

kable(estadisticas_tcl_poisson, digits = 4,
      caption = "Convergencia de estadísticos de la media muestral para Poisson",
      col.names = c("n", "Media Obs.", "Media Teór.", "Var. Obs.", "Var. Teór.", 
                   "SD Obs.", "SD Teór.", "Error Media", "Error Var.", "Error SD"))

# Visualización de la convergencia
ggplot(data_tcl_poisson, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha
 = 0.7, fill = "#951013") +
  geom_vline(xintercept = lambda_tcl_poisson, color = "#210405", linetype = "dashed", linewidth = 1) +
  facet_wrap(~n_label, scales = "free_y") +
  labs(title = "Distribución de Medias Muestrales (TCL) para Poisson",
       subtitle = paste("Distribución original: Poisson(λ =", lambda_tcl_poisson, "). Línea roja: media teórica"),
       x = "Media muestral",
       y = "Densidad") +
  theme_minimal()
```
...
# Estadísticas
stats_suma <- data.frame(
  Distribución = c("Suma de Poisson", "Poisson(λ_total)", "Diferencia"),
  Media = c(mean(suma_simulada), mean(poisson_directa), abs(mean(suma_simulada) - mean(poisson_directa))),
  Varianza = c(var(suma_simulada), var(poisson_directa),
               abs(var(suma_simulada) - var(poisson_directa)))
)
kable(stats_suma, digits = 4, 
      caption = "Estadísticas para suma de dos variables Poisson")

data_suma <- data.frame(
  valores = c(suma_simulada, poisson_directa),
  tipo = rep(c("Suma de Poisson", "Poisson(λ_total)"), each = n_sim)
)
ggplot(data_suma, aes(x = valores, fill = tipo)) +
  geom_histogram(alpha = 0.6, binwidth = 1, center=0, position = "dodge2") +
  scale_x_continuous(breaks = seq(0, max(c(suma_simulada, poisson_directa)), by = 1)) +
  labs(title = "Verificación de la Propiedad Aditiva de Poisson",
       subtitle = paste("λ1 =", lambda1, ", λ2 =", lambda2, ", λ_total =", lambda_total),
       x = "Número de eventos",
       y = "Frecuencia",
       fill = "Tipo") +
  theme_minimal() +
  scale_fill_manual(values = c("#131095", "#050421"))
```
...
lambda_vars <- c(1.2, 2.5, 3.3, 0.8)
lambda_total_4 <- sum(lambda_vars)
set.seed(987)
X_vars <- sapply(lambda_vars, function(lam) rpois(n_sim, lam))
suma_4_simulada <- rowSums(X_vars)
poisson_4_directa <- rpois(n_sim, lambda_total_4)

# Estadísticas
stats_suma_4 <- data.frame(
  Distribución = c("Suma de 4 Poisson", "Poisson(λ_total)", "Diferencia"),
  Media = c(mean(suma_4_simulada), mean(poisson_4_directa), abs(mean(suma_4_simulada) - mean(poisson_4_directa))),
  Varianza = c(var(suma_4_simulada), var(poisson_4_directa),
               abs(var(suma_4_simulada) - var(poisson_4_directa)))
)
kable(stats_suma_4, digits = 4, 
      caption = "Estadísticas para suma de cuatro variables Poisson")

data_suma_4 <- data.frame(
  valores = c(suma_4_simulada, poisson_4_directa),
  tipo = rep(c("Suma de 4 Poisson", "Poisson(λ_total)"), each = n_sim)
)
ggplot(data_suma_4, aes(x = valores, fill = tipo)) +
  geom_histogram(alpha = 0.6, binwidth = 1, center=0, position = "dodge2") +
  scale_x_continuous(breaks = seq(0, max(c(suma_4_simulada, poisson_4_directa)), by = 1)) +
  labs(title = "Verificación de la Propiedad Aditiva de Poisson para 4 Variables",
       subtitle = paste("λ total =", lambda_total_4),
       x = "Número de eventos",
       y = "Frecuencia",
       fill = "Tipo") +
  theme_minimal() +
  scale_fill_manual(values = c("#109513", "#040521"))
```
:::
...
lambda_visitantes <- 15
capacidad <- 25
prob_degradacion <- 1 - ppois(capacidad, lambda_visitantes)
cat("1. P(Degradación en 1 minuto) =", round(prob_degradacion, 4), "(", round(prob_degradacion*100, 2), "%)\n")
```
...
prob_degradacion_duplicada <- 1 - ppois(capacidad_duplicada, lambda_visitantes)
cat("2. P(Degradación con capacidad duplicada) =", round(prob_degradacion_duplicada, 4), "(", round(prob_degradacion_duplicada*100, 2), "%)\n")
```
...
capacidad_optima <- 0
while(TRUE) {
  prob_degradacion_optima <- 1 - ppois(capacidad_optima, lambda_visitantes)
  if(prob_degradacion_optima < 0.01) {
    break
  }
  capacidad_optima <- capacidad_optima + 1
}
cat("3. Capacidad óptima para P(Degradación) < 1%:", capacidad_optima, "\n")
```
...
set.seed(111)
n_minutos <- 60
sim_visitantes_hora <- rpois(n_minutos, lambda_visitantes)
degradacion_hora <- sum(sim_visitantes_hora > capacidad) / n_minutos
cat("4. Porcentaje de tiempo con degradación en 1 hora:", round(degradacion_hora * 100, 2), "%\n")
```
:::
...
prob_agotarse <- 1 - ppois(inventario_inicial, lambda_ventas)
cat("1. P(Sin existencias) =", round(prob_agotarse, 4), "(", round(prob_agotarse*100, 2), "%)\n")
```
...
inventario_optimo <- 0
while(TRUE) {
  prob_agotarse_optimo <- 1 - ppois(inventario_optimo, lambda_ventas)
  if(prob_agotarse_optimo < 0.05) {
    break
  }
  inventario_optimo <- inventario_optimo + 1
}
cat("2. Unidades necesarias para P(Sin existencias) < 5%:", inventario_optimo, "\n")
```
...
costo_diario_esperado <- function(inventario) {
  costo_total <- 0
  for(ventas in 0:20) {
    prob_ventas <- dpois(ventas, lambda_ventas)
    unidades_vendidas <- min(ventas, inventario)
    unidades_no_vendidas <- inventario - unidades_vendidas
    ventas_perdidas <- max(0, ventas - inventario)
    
    costo_total <- costo_total + prob_ventas * (
      (unidades_no_vendidas * costo_mantenimiento) +
...
inventarios_posibles <- 0:20
costos_esperados <- sapply(inventarios_posibles, costo_diario_esperado)
costo_optimo <- min(costos_esperados)
inventario_n_optimo <- inventarios_posibles[which.min(costos_esperados)]
cat("3. Costo diario esperado mínimo: $", round(costo_optimo, 2), "con inventario de", inventario_n_optimo, "unidades\n")
data_costos  <- data.frame(
  inventario = inventarios_posibles,
  costo_esperado = costos_esperados
)

ggplot(data_costos, aes(x = inventario, y = costo_esperado)) +
  geom_line(color = "#130495", linewidth = 1.2) +
  geom_point(color = "#E74C3C", linewidth = 1.2) +
  labs(title = "Costo Diario Esperado vs Nivel de Inventario",
       x = "Nivel de Inventario (unidades)",
       y = "Costo Diario Esperado ($)") +
  theme_minimal()
```
d)  Determine el nivel óptimo de inventario que minimiza el costo total esperado.

```{r}
cat("4. Nivel óptimo de inventario para minimizar costos:", inventario_n_optimo, "\n")
```
:::
