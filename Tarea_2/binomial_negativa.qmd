...
geometrica_binneg <- function(r = 4, p = 0.3, n_sim) { 
...
ggplot(data_hist, aes(x=valores, fill=distribucion))+
  ...
  theme_minimal()+
  theme(legend.position = "none")+
  scale_fill_manual(values = c("#131095", "#210405"))
}

geometrica_binneg(r = 3, p = 0.4, n_sim = 1000)
```
:::
...
cat("Número esperado de tiros fallados (teórico):", round(prob_f, 2),"±", round(desv_std_f, 2), "\n")
...
ggplot(data.frame(Tiros = sumgeom_f), aes(x = Tiros)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "#010997", alpha = 0.7) +
  geom_vline(xintercept = prob_f, color = "#13BBBB", 
             linetype = "dashed", linewidth = 1) +
...
ggplot(data.frame(iteraciones = sum_fallidas), aes(x = iteraciones)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "#AA8", alpha = 0.7) +
  geom_vline(xintercept = costo_fallidas, color = "#D95C7C", 
             linetype = "dashed", linewidth = 1) +
